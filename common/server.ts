    ...
    private getDescription(params: ToolParams): string {
        ...
        if (zodSchema && zodSchema instanceof ZodObject) {
            ...
            for (const key of Object.keys(zodSchema.shape)) {
                ...
                description += `- ${key} (${this.getReadableTypeName(field)})${field.isOptional() ? '' : ' *required*'}${field.description ? `: ${field.description}` : ''}${key === "examples" && field instanceof ZodEnum ? ` (e.g. ${Object.keys(field.enum).join(', ')})` : ''}${key === "constraints" && field instanceof ZodEnum ? `\n  - ${Object.keys(field.enum).join('\n  - ')}` : ''} \n`;
            }
        }
