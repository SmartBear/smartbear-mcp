...
> **Need access to Reflect, QTM4J, PactFlow, or Collaborator?** These products are only available in the [Local MCP Server](/smartbear-mcp/docs/local-server), which provides all products in a single npm package.
...
If you need all products in a single connection, use the [Local MCP Server](/smartbear-mcp/docs/local-server) instead.
