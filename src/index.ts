...
async function main() {
  ...
  if (pactBrokerUrl) {
    if (pactBrokerToken) {
      server.addClient(new PactflowClient(pactBrokerToken, pactBrokerUrl, "pactflow", server));
      client_defined = true;
    } else if (pactBrokerUsername && pactBrokerPassword) {
      server.addClient(new PactflowClient({ username: pactBrokerUsername, password: pactBrokerPassword }, pactBrokerUrl, "pact_broker", server));
      client_defined = true;
    } else {
