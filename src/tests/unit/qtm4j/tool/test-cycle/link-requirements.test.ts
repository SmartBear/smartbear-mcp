...
import { LinkRequirementsToCycle } from "../../../../../qtm4j/tool/test-cycle/link-requirements";
...
describe("LinkRequirementsToCycle", () => {
  ...
  let instance: LinkRequirementsToCycle;
  ...
  beforeEach(() => {
    ...
    instance = new LinkRequirementsToCycle(mockClient as any);
  });
  ...
  describe("handle", () => {
    it("should resolve cycle and requirement keys and link", async () => {
    ...
    it("should link via JQL filter", async () => {
      mockCycleResolver.resolveAndReturn.mockResolvedValueOnce({
        "SCRUM-TR-1": { uid: "cycle-uid-abc" },
      });
