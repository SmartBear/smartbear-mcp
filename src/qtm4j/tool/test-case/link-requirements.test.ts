...
import { LinkRequirements } from "./link-requirements.ts";

type MockMethods = Record<string, ReturnType<typeof vi.fn>>;

describe("LinkRequirements", () => {
  ...
  let mockReqResolver: MockMethods;
  let instance: LinkRequirements;
  ...
  beforeEach(() => {
    ...
    instance = new LinkRequirements(mockClient as Qtm4jClient);
  });
  ...
  describe("handle", () => {
    it("should resolve test case key and link requirements by key", async () => {
      ...
      expect(mockApiClient.post).toHaveBeenCalledWith(
        ENDPOINTS.LINK_REQUIREMENTS("uid-abc", 2),
    ...
    it("should use latest version when versionNo not provided", async () => {
      mockTcResolver.resolveAndReturn.mockResolvedValueOnce({
      ...
      expect(mockApiClient.post).toHaveBeenCalledWith(
        ENDPOINTS.LINK_REQUIREMENTS("uid-abc", 3),
        expect.any(Object),
      );
      expect(result.structuredContent.versionNo).toBe(3);
    });
    ...
    it("should use provided versionNo instead of latest", async () => {
      ...
      expect(mockApiClient.post).toHaveBeenCalledWith(
        ENDPOINTS.LINK_REQUIREMENTS("uid-abc", 1),
        expect.any(Object),
      );
      expect(result.structuredContent.versionNo).toBe(1);
    });

    it("should link requirements via JQL filter", async () => {
      mockTcResolver.resolveAndReturn.mockResolvedValueOnce({
        "SCRUM-TC-145": { uid: "uid-abc", latestVersion: 2 },
      });
    ...
    it("should call post with an empty body when all requirement keys fail to resolve", async () => {
      ...
      const result = await instance.handle({
        key: "SCRUM-TC-145",
        requirementKeys: ["SCRUM-999"],
      });

      expect(mockApiClient.post).toHaveBeenCalledWith(expect.any(String), {});
      expect(result.content[0].text).toContain("SCRUM-999");
    });
