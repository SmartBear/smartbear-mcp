          ...
          try {
            ...
            switch (tool.handler) {
              ...
              case 'deletePortal':
                ...
                break;
              case 'updatePortal': {
                const { portalId: updatePortalId, ...updatePortalBody } = args as any;
                result = await this.updatePortal(updatePortalId, updatePortalBody as UpdatePortalArgs);
                break;
              }
              ...
              case 'createPortalProduct': {
                const { portalId: createProductPortalId, ...createProductBody } = args as any;
                result = await this.createPortalProduct(createProductPortalId, createProductBody as CreateProductArgs);
                break;
              }
              ...
              case 'updatePortalProduct': {
                const { productId: updateProductId, ...updateProductBody } = args as any;
                result = await this.updatePortalProduct(updateProductId, updateProductBody as UpdateProductArgs);
                break;
              }
              default:
            ...
            const formattedResult = tool.formatResponse ? tool.formatResponse(result) : result;
