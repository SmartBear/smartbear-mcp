...
export const TESTSUITE_TOOLS: QMetryToolParams[] = [
  {
    ...
    purpose:
      "REQUIRED WORKFLOW: Before calling this tool, ALWAYS call 'Fetch UDF Layout' (entityType='TS', pageName='ADD') first. " +
    ...
    hints: [
      ...
      "╚══════════════════════════════════════════════════════════════════╝",
      "NEVER call 'Create Test Suite' without first calling 'Fetch UDF Layout' with entityType='TS', pageName='ADD'.",
      ...
      "Before creating any test suite, call 'Fetch UDF Layout' with entityType='TS', pageName='ADD'.",
      "Response keys use 'TS' — same structure as TC but scoped to test suite module.",
      "",
      "SYSTEM FIELDS mandatory check — use 'systemFields' array (from qmSDF.TS in newlayout):",
      ...
      "",
      "UDF FIELDS mandatory check — use 'fields' array (from qmUDF.TS in newlayout):",
      "  Each entry: { name, label, fieldTypeName, isMandatory, listName? }",
      "  isMandatory=true means field MUST have a value.",
      "",
      "DEFAULT VALUES — use 'defaultValues' object (from qmDefaultValue.TS in newlayout):",
      "  Shape: { fieldName: defaultValueId }  e.g. { 'priority': 5232497, 'estimatedTime': 18305 }",
      ...
      "  Example: defaultValues = { priority: 5232497, estimatedTime: 18305 }",
      "    → user only said 'create a test suite named X'",
      "    → payload MUST include: priority=5232497, estimatedTime=18305",
      ...
      "  4. For matched system fields (e.g. 'Status'): check 'customListObjs.testSuiteState' from project info.",
      ...
      "",
      "╔══════════════════════════════════════════════════════════════════╗",
      "║  POST-CREATE MANDATORY: Link Platform to Test Suite              ║",
      "╚══════════════════════════════════════════════════════════════════╝",
      "AFTER every successful test suite creation, ALWAYS call 'Link Platforms to Test Suite'.",
      "Reason: QMetry UI enforces platform association — test suites without a linked platform",
      "are invalid for execution and will show validation errors in the UI.",
      "STEP 1: Call 'Fetch Platforms' to get available platforms for the project.",
      "STEP 2: If the user specified a platform, match by name and use its platformID.",
      "        If no platform was specified, use the first available non-archived platform.",
      "STEP 3: Call 'Link Platforms to Test Suite' with qmTsId=<new suite id> and qmPlatformId=<platformID>.",
      "NEVER report the test suite as successfully created without completing this platform link step.",
      "╚══════════════════════════════════════════════════════════════════╝",
    ],
    outputDescription:
  ...
  },
  {
    ...
    purpose:
      "REQUIRED WORKFLOW: Before calling this tool, call 'Fetch UDF Layout' (entityType='TS', pageName='DETAIL') to discover mandatory fields, default values, and valid list option IDs. " +
    ...
    hints: [
      ...
      "ALWAYS call 'Fetch UDF Layout' with entityType='TS', pageName='DETAIL' before updating.",
      ...
      "  This applies to non-mandatory fields too (e.g. testSuiteState). Omitting them = data loss.",
      "systemFields (from qmSDF.TS): isMandatory=true fields must retain a valid value after update.",
      "fields/UDF (from qmUDF.TS): isMandatory=true UDF fields must be included if being changed.",
      "=== END DEFAULT VALUES ===",
      "",
      ...
      "   Example: udfFields: { ts_field: 'value' }, UDF: { ts_field: { fieldID: 2001, value: 'value' } }",
  ...
  },
  {
    ...
    hints: [
      ...
      "=== SYSTEM FIELD FILTER (filter) — FORMAT GUIDE ===",
      "filter is a JSON array string. Each object filters by one system field. Multiple conditions use AND logic.",
      "NO 'udfmID' in system field filters — that is only for udfFilter.",
      "value can be empty string '' when no value selected (open-ended filter).",
      'Base structure: {"type":"string"|"list"|"numeric"|"date", "field":"<fieldKey>", "value":<value> [,"comparison":"between"|"gt"|"lt"]}',
      "",
      "TYPES:",
      "  CRITICAL — list type: value MUST be numeric IDs. NEVER use string names like 'Open', 'Active' — API returns 400. Get IDs from project info customListObjs.",
      '  string:  {"type":"string","field":"name","value":"trst"}  — partial match',
      '  list:    {"type":"list","field":"owner","value":[<id1>,<id2>]}  — numeric IDs only; value can be ""',
      '  numeric: {"type":"numeric","field":"attachmentCount","value":3}',
      '  date:    {"type":"date","field":"createdDate","comparison":"gt"|"lt","value":"<date-in-project-format>"}  — value can be ""',
      "",
      "FILTER FIELDS:",
      "  string:  name, entityKeyId",
      "  list:    release, cycle, platform, testsuiteStatus, createdByAlias, updatedByAlias, owner,",
      "           isArchived (0=active / 1=archived), linkedPlatformCount, linkedTcCount",
      "  numeric: attachmentCount, remExecutionTime, totalExecutionTime, totalunassignedtcruns",
      "  date:    createdDate, updatedDate — ALWAYS two objects (gt + lt). Single date: use SAME date for both gt and lt. Date range: first date=gt, second date=lt.",
      "",
      "IDs: release/cycle from FETCH_RELEASES_AND_CYCLES; platform from FETCH_PLATFORMS; status/owner etc. from project info.",
      "",
      "ARCHIVE FILTER — INTENT MAPPING:",
      "  User says 'show archived' / 'only archived'  → value:[1]   — archived only",
      "  User says 'include archived' / 'all including archived'  → value:[0,1]  — active + archived",
      "  User says 'active only' / no mention of archive  → omit isArchived filter (API default = active)",
      '  Filter object: {"type":"list","value":[0,1],"field":"isArchived"}  (0=active, 1=archived)',
      "=== END SYSTEM FIELD FILTER GUIDE ===",
      "",
      "SORT FIELDS: entityKey, name, testsuiteStatus, linkedPlatformCount, linkedTcCount, createdDate, createdByAlias, updatedDate, updatedByAlias, attachmentCount, remExecutionTime, totalExecutionTime",
      "Use empty string '' as folderPath for root directory",
      "",
      "COMBINED FILTER CRITICAL RULE: When user conditions span BOTH system fields (filter) AND UDF fields (udfFilter), pass BOTH parameters in a SINGLE API call. NEVER split into two separate API calls. Example: user asks for 'release=X AND custom field=Y' → one call with filter='[{release}]' AND udfFilter='[{custom}]' together.",
      "=== UDF FILTER (udfFilter) — FORMAT GUIDE ===",
      "udfFilter filters test suites by UDF field values. JSON array string. Multiple conditions use AND logic.",
      "CRITICAL: Every filter object MUST include 'udfmID' (projectUserFieldID). Get from 'Fetch UDF Layout' (entityType='TS'): fields[].projectUserFieldID. NEVER guess or reuse udfmID from memory/context — always fetch fresh from UDF Layout.",
      "CRITICAL: 'field' = UDF field key from fields[].name.",
      '{"type":"list","field":"<key>","value":[<id1>],"udfmID":<id>}  — single/multi-select lookup',
      '{"type":"list","field":"<key>","value":[<parentId>,<childId>],"udfmID":<id>,"isCascading":true}  — cascading dropdown. CRITICAL: value array MUST contain BOTH parent ID and child ID. Parent ID first, child ID second. Example: user says "React - Redux" → value:[reactId, reduxId]. NEVER send only child ID.',
      '{"type":"string","field":"<key>","value":"<text>","udfmID":<id>}  — text or large text',
      '{"type":"date","field":"<key>","value":"dd-MMM-yyyy","comparison":"gt"|"lt","udfmID":<id>}  — date (comparison REQUIRED, format: 16-Sep-2026). CRITICAL: UDF date filter ALWAYS requires TWO objects for the same field — one with comparison:"gt" and one with comparison:"lt". NEVER send only one date object for a UDF date field. DATE RULES: (1) Single date from user (e.g. "17 Aug 2026") → use SAME date for both: gt=17-Aug-2026 AND lt=17-Aug-2026. (2) Date range from user (e.g. "Aug 1 to Aug 5") → use first as gt=01-Aug-2026 and second as lt=05-Aug-2026.',
      "KNOWN API BUG — DATE UDF + OTHER UDF FILTERS = 400: Including a date type in udfFilter alongside any other UDF filter type (string/list/numeric) causes API to return 400 CO.GENERAL_EXCEPTION. MANDATORY WORKAROUND: (1) Remove ALL date UDF conditions from udfFilter entirely. (2) Fetch with system filter + remaining non-date UDF filters in one call. (3) After receiving results, verify date field value in each returned record against user's date criteria. NEVER attempt to include date in udfFilter when other UDF filters are also present.",
      '{"type":"numeric","field":"<key>","value":<number>,"udfmID":<id>}  — number',
      'Full example: \'[{"type":"list","value":[5232621],"udfmID":2637526,"field":"multiSelect19"},{"type":"list","value":[5232623,5232625],"field":"project19","udfmID":2637580,"isCascading":true},{"type":"string","value":"Test","udfmID":2637427,"field":"str1"},{"type":"date","value":"16-Sep-2026","field":"date19","comparison":"gt","udfmID":2637562},{"type":"date","value":"17-Sep-2026","field":"date19","comparison":"lt","udfmID":2637562},{"type":"string","value":"Jaipur","udfmID":2637445,"field":"largeText19"},{"type":"list","value":[5232630,5232631],"udfmID":2637508,"field":"lookup19"},{"type":"numeric","value":2,"udfmID":2637544,"field":"age19"}]\'',
      "=== END UDF FILTER GUIDE ===",
    ],
    outputDescription:
  ...
  },
  {
    ...
    examples: [
      ...
      {
        description: "Get all test suites including archived",
        parameters: {
          tsFolderID: 113557,
          filter: '[{"value":[0,1],"type":"list","field":"isArchived"}]',
        },
        expectedOutput: "List of active and archived test suites",
      },
      {
        description: "Get only archived test suites",
        parameters: {
          tsFolderID: 113557,
          filter: '[{"value":[1],"type":"list","field":"isArchived"}]',
        },
        expectedOutput: "List of only archived test suites",
      },
      {
    ...
    examples: [
      ...
      },
      {
        ...
        parameters: {
          ...
          viewId: 79451,
          tcrUdfFilter:
            '[{"type":"date","field":"PGTE_DATEPICKER","comparison":"gt","value":"01-Jun-2026"},{"type":"date","field":"PGTE_DATEPICKER","comparison":"lt","value":"30-Jun-2026"}]',
        },
        expectedOutput:
      ...
      {
        description: "Show only test case runs with linked defects (bugs)",
        parameters: {
          tsrunID: "107021",
          viewId: 6887,
        ...
        expectedOutput:
          "Test case runs that have at least one linked defect/bug",
      },
      {
        description:
          "Show only test case runs with linked defects, also filtered by Test Run UDF list value",
        parameters: {
          tsrunID: "107021",
          viewId: 6887,
          showTcWithDefects: true,
      ...
      {
        description: "Filter test case runs by execution status",
        parameters: {
          tsrunID: "2362144",
          viewId: 104123,
          filter: '[{"type":"string","value":"Failed","field":"runStatus"}]',
        ...
        expectedOutput:
          "Only Failed test case runs for the given test suite run",
      },
    ],
    hints: [
      ...
      'Format: {"type":"list", "field":"<fieldKey>", "value":[<parentId>,<childId>], "isCascading":true}',
      'Example: [{"type":"list","value":[5126498,5126499],"field":"cascade_vK","isCascading":true}]',
      "CRITICAL: value array MUST contain BOTH parent ID (first) and child ID (second). Example: user says 'React - Redux' → value:[reactId, reduxId]. NEVER send only child ID.",
      ...
      "value: date string in the project's active date format (fetch from 'Fetch QMetry Project Info' → dateTimeFormatID → dateTimeFormatNew[].unique_value), or empty string '' to leave the bound open.",
      'Format: {"type":"date", "field":"<fieldKey>", "comparison":"gt"|"lt", "value":"<date-in-project-format>"}',
      'Example — date range: [{"type":"date","field":"PGTE_DATEPICKER","comparison":"gt","value":"<date>"},{"type":"date","field":"PGTE_DATEPICKER","comparison":"lt","value":"<date>"}]',
      'Example — open-ended (no upper bound): [{"type":"date","field":"PGTE_DATEPICKER","comparison":"gt","value":"<date>"}]',
      "To filter by a single date use two conditions with the SAME date for both: gt=<date> AND lt=<date>. For a date range, use gt=<startDate> and lt=<endDate>.",
      ...
      '  {"type":"date","field":"PGTE_DATEPICKER","comparison":"gt","value":"<date-in-project-format>"},',
      '  {"type":"date","field":"PGTE_DATEPICKER","comparison":"lt","value":"<date-in-project-format>"},',
      '  {"type":"numeric","value":2,"field":"NB_number_TR"}',
      "]",
      "--- FILTER PARAMETER SUMMARY ---",
      "tcrUdfFilter: filter by Test Run UDF fields (this section)",
      "udfFilter: filter by Test Case UDF fields (different from test run UDFs) — REQUIRES 'udfmID' in each object. See Fetch Test Cases hints for full format guide.",
      "filter: filter by standard execution fields — JSON array string. Multiple conditions = AND logic.",
      "  EXECUTION FILTER FIELDS:",
      "    string:  runStatus (e.g. 'Passed', 'Failed', 'Not Run', 'In Progress', 'Blocked')",
      "             executedBy (username/display name string)",
      "             entityKeyId (test case key, e.g. 'MAC-TC-5'; comma-separated for multiple)",
      "    list:    release (numeric IDs), cycle (numeric IDs), platform (numeric IDs)",
      "    date:    executedDate — ALWAYS two objects (gt + lt). Single date: same value for both.",
      '  Example: \'[{"type":"string","value":"Failed","field":"runStatus"}]\'',
      '  Example: \'[{"type":"list","value":[55178],"field":"release"},{"type":"string","value":"Passed","field":"runStatus"}]\'',
      "showTcWithDefects: boolean (default false) — when true, returns only test case runs that have linked defects.",
      "  INTENT MAPPING:",
      "    'show test cases with bugs' / 'with defects' / 'linked to defects' / 'has bugs'  → showTcWithDefects: true",
      "    No mention of defects / bugs  → omit (defaults to false)",
      "All three filter params (filter, udfFilter, tcrUdfFilter) plus showTcWithDefects can be combined in a single request.",
      "",
