...
export const REQUIREMENT_TOOLS: QMetryToolParams[] = [
  ...
  },
  {
    ...
    examples: [
      ...
      {
        description:
          "Filter requirements by state using numeric ID (resolve state ID from project info customListObjs)",
        parameters: {
          projectKey: "MAC",
          filter:
            '[{"type":"list","value":[688912],"field":"requirementStateAlias"}]',
        },
        expectedOutput:
          "Requirements with the specified state (e.g., 'Open'). Get state ID from project info customListObjs — match by name.",
      },
      {
    ...
    hints: [
      ...
      "CRITICAL FIELD NAME DISAMBIGUATION: For FILTERS, status field is 'requirementStateAlias' (with Alias suffix). 'requirementState' (without Alias) is ONLY used in CREATE/UPDATE payloads — NEVER in filter. Always use 'requirementStateAlias' in filter.",
      "filter: JSON array string. Multiple conditions = AND logic.",
      'String fields: {"type":"string","field":"<name>","value":"<text>"}',
      "  String fields: name, entityKeyId",
      "CRITICAL — list type: value MUST be numeric IDs. NEVER use string names like 'Open', 'Critical' — API returns 400. Get IDs from project info customListObjs.",
      'List fields (numeric IDs): {"type":"list","field":"<name>","value":[<id1>,<id2>]}',
      "  List (numeric ID) fields: release, cycle, isArchived (0=active / 1=archived), priorityAlias, createdByAlias, updatedByAlias, requirementStateAlias, componentAlias (use -1 for 'None')",
      "  Get release/cycle IDs from fetch_releases_and_cycles tool OR from project info projects[].releases[].releaseID / builds[].buildID.",
      "  Get priorityAlias/requirementStateAlias/createdByAlias/updatedByAlias/componentAlias IDs from project info customListObjs.*[].id (match by name).",
      'List fields (string values): {"type":"list","field":"<name>","value":["<val>"]}',
      "  createdSystem: values are 'QMetry', 'JIRA', 'AZURE'",
      "  owner: values are display names e.g. 'admin', 'Varis Khan'",
      'Numeric fields: {"type":"numeric","field":"<name>","value":<number>}',
      "  Numeric fields: attachmentCount, linkedTcCount, linkedDfCount",
      'Date fields: {"type":"date","field":"<name>","value":"<project-date-format>","comparison":"gt"|"lt"}',
      "  Date fields: createdDate, updatedDate. Use project's active date format from Fetch QMetry Project Info → dateTimeFormatID → dateTimeFormatNew[].unique_value",
      "  Use two date objects (gt + lt) for between-range filtering",
      'Example multi-filter: \'[{"type":"list","field":"release","value":[55178]},{"type":"list","field":"requirementStateAlias","value":[101,102]},{"type":"string","field":"name","value":"Login"},{"type":"date","field":"createdDate","value":"16-Sep-2026","comparison":"gt"},{"type":"date","field":"createdDate","value":"16-Sep-2026","comparison":"lt"}]\'',
      "",
      ...
      "",
      "COMBINED FILTER CRITICAL RULE: When user conditions span BOTH system fields (filter) AND UDF fields (udfFilter), pass BOTH parameters in a SINGLE API call. NEVER split into two separate API calls. Example: user asks for 'status=Open AND custom field=Y' → one call with filter='[{requirementStateAlias}]' AND udfFilter='[{custom}]' together.",
      "=== UDF FILTER (udfFilter) — FORMAT GUIDE ===",
      "udfFilter filters requirements by UDF field values. JSON array string. Multiple conditions use AND logic.",
      "CRITICAL: Every filter object MUST include 'udfmID' (projectUserFieldID). Get from 'Fetch UDF Layout' (entityType='RQ'): fields[].projectUserFieldID. NEVER guess or reuse udfmID from memory/context — always fetch fresh from UDF Layout.",
