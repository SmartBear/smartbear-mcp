...
export const TESTCASE_TOOLS: QMetryToolParams[] = [
  ...
  },
  {
    ...
    examples: [
      ...
      {
        description:
          "Filter by status (Closed), type (UI), and priority (Critical) — NOTE: filter uses Alias suffix: testCaseStateAlias, testCaseTypeAlias, priorityAlias",
        parameters: {
          filter:
            '[{"type":"list","value":[5232556],"field":"testCaseStateAlias"},{"type":"list","value":[5232507],"field":"testCaseTypeAlias"},{"type":"list","value":[5232496],"field":"priorityAlias"}]',
        },
        expectedOutput:
          "Test cases with Closed status, UI type, and Critical priority. IDs resolved from project info customListObjs.",
      },
      {
        description: "Get all test cases including archived",
        parameters: {
          filter: '[{"type":"list","value":[0,1],"field":"isArchived"}]',
        },
        expectedOutput: "List of active and archived test cases",
      },
      {
        description: "Get only archived test cases",
        parameters: {
          filter: '[{"type":"list","value":[1],"field":"isArchived"}]',
        },
        expectedOutput: "List of only archived test cases",
      },
      {
        description: "Get only parameterized test cases",
        parameters: { isParameterized: true },
        expectedOutput: "List of parameterized test cases only",
      },
      {
        description: "Get test cases that have shared (reusable) test cases",
        parameters: { havingSharedTestcases: true },
        expectedOutput: "List of test cases containing shared test cases",
      },
      {
        description:
          "Get only shareable test cases (via isShared filter field)",
        parameters: {
          filter: '[{"type":"list","value":[1],"field":"isShared"}]',
        },
        expectedOutput: "List of shareable/shared test cases only",
      },
      {
        description: "Get only AI/QI-generated test cases",
        parameters: {
          filter: '[{"type":"list","value":[1],"field":"isFromQIGenerated"}]',
        },
        expectedOutput: "List of test cases generated by QI (AI)",
      },
      {
        description:
          "Get only archived AND QI-generated test cases (combined filters)",
        parameters: {
          filter:
            '[{"type":"list","value":[1],"field":"isArchived"},{"type":"list","value":[1],"field":"isFromQIGenerated"}]',
        },
        expectedOutput: "List of archived test cases that were AI/QI-generated",
      },
      {
        description:
          "Get test cases not assigned to any folder (root/unassigned only)",
        parameters: { showRootOnly: true },
        expectedOutput:
          "List of test cases that are not assigned to any folder",
      },
      {
        description:
          "Combined system + UDF filter in ONE call: status=Closed, priority=Critical (system filter) AND first name=PK, birthdate=22-Sep-2026 (UDF filter)",
        parameters: {
          filter:
            '[{"type":"list","value":[5232556],"field":"testCaseStateAlias"},{"type":"list","value":[5232496],"field":"priorityAlias"}]',
          udfFilter:
            '[{"type":"string","value":"PK","udfmID":2637427,"field":"str1"},{"type":"date","value":"22-Sep-2026","comparison":"gt","udfmID":2637562,"field":"date19"},{"type":"date","value":"22-Sep-2026","comparison":"lt","udfmID":2637562,"field":"date19"}]',
        },
        expectedOutput:
          "Test cases matching all conditions. System + UDF filters combined in single API call. Single date uses same value for both gt and lt.",
      },
    ],
    hints: [
      ...
      "value can be empty string '' when no value is selected (open-ended filter).",
      'Base structure: {"type":"string"|"list"|"numeric"|"date", "field":"<fieldKey>", "value":<value> [,"comparison":"between"|"gt"|"lt"]}',
      "",
      "╔══════════════════════════════════════════════════════════════════════════════╗",
      "║  FILTER FIELD NAME MAP — CREATE/UPDATE uses different names than FILTER    ║",
      "║  FILTER field name      ←→  CREATE/UPDATE field name                       ║",
      "║  testCaseStateAlias     ←→  testCaseState   (FILTER MUST use Alias suffix) ║",
      "║  testCaseTypeAlias      ←→  testCaseType    (FILTER MUST use Alias suffix) ║",
      "║  priorityAlias          ←→  priority        (FILTER MUST use Alias suffix) ║",
      "║  testingTypeAlias       ←→  testingType     (FILTER MUST use Alias suffix) ║",
      "║  componentAlias         ←→  component       (FILTER MUST use Alias suffix) ║",
      "║  createdByAlias         ←→  createdBy       (FILTER MUST use Alias suffix) ║",
      "║  updatedByAlias         ←→  updatedBy       (FILTER MUST use Alias suffix) ║",
      "║  NEVER use the CREATE/UPDATE name in filter — API returns 400 error.       ║",
      "╚══════════════════════════════════════════════════════════════════════════════╝",
      "",
      "TYPES:",
      "  CRITICAL — list type: value MUST be numeric IDs. NEVER use string names like 'Critical', 'Closed', 'UI' — API returns 400. Get IDs from project info customListObjs.",
      '  string:  {"type":"string","field":"entityKeyId","value":"VKMCP-TC-53"}  — comma-separated for multiple keys',
      '  list:    {"type":"list","field":"owner","value":[<id1>,<id2>]}  — numeric IDs only; value can be "" if empty',
      '  numeric: {"type":"numeric","field":"linkedRqCount","value":2}',
      '  date:    {"type":"date","field":"createdDate","comparison":"gt"|"lt","value":"<date-in-project-format>"}  — value can be "" for open bound',
      "",
      "FILTER FIELDS (use EXACTLY these names — with Alias suffix where shown):",
      "  string:  entityKeyId",
      "  list:    release, cycle, owner, componentAlias, testCaseTypeAlias, testingTypeAlias,",
      "           testCaseStateAlias, priorityAlias, createdByAlias, updatedByAlias,",
      "           isSyncWithParent (1=synced/0=not), latestExecutionStatus (-1=Not Run),",
      "           successRate (use with comparison:'between'),",
      "           isArchived (0=active / 1=archived),",
      "           isShared (1=shared/shareable test cases only),",
      "           isFromQIGenerated (1=AI/QI-generated test cases only)",
      "  numeric: linkedRqCount, executionMinutes, attachmentCount",
      ...
      "",
      "IDs: get release/cycle from FETCH_RELEASES_AND_CYCLES; owner/componentAlias/state etc. from project info customListObjs.",
      'Correct filter example: \'[{"type":"list","value":[5232556],"field":"testCaseStateAlias"},{"type":"list","value":[5232507],"field":"testCaseTypeAlias"},{"type":"list","value":[5232496],"field":"priorityAlias"}]\'',
      "",
      ...
      "",
      "SPECIAL TOP-LEVEL PARAMS (NOT filter fields — pass as direct schema params):",
      "  isParameterized: true  — 'show only parameterized test cases' / 'parameterized only'",
      "  havingSharedTestcases: true  — 'show test cases with shared test cases' / 'having shared'",
      "  showRootOnly: true  — 'show test cases not assigned to any folder' / 'unassigned to folder' / 'root only'",
      '  isShared filter  — \'show only shareable test cases\' / \'shared test cases only\' → add filter field: {"type":"list","value":[1],"field":"isShared"}',
      '  isFromQIGenerated filter  — \'show AI-generated\' / \'QI generated\' / \'generated by QI\' → add filter field: {"type":"list","value":[1],"field":"isFromQIGenerated"}',
      "=== END SYSTEM FIELD FILTER GUIDE ===",
      "COMBINED FILTER CRITICAL RULE: When user conditions span BOTH system fields (filter) AND UDF fields (udfFilter), pass BOTH parameters in a SINGLE API call. NEVER split into two separate API calls. Example: user asks for 'status=Open AND first name=vk' → one call with filter='[{testCaseStateAlias}]' AND udfFilter='[{str1}]' together.",
      "",
      ...
      "udfFilter filters test cases by UDF field values. JSON array string. Multiple conditions use AND logic.",
      "CRITICAL: Every filter object MUST include 'udfmID'. CORRECT SOURCE: filterTemplate[].udfmID from fetch_test_cases list response (where isUDF===true). Do NOT use fieldID from Fetch UDF Layout — that is a different value and will cause 400 errors. Workflow: (1) Call fetch_test_cases with no udfFilter to get filterTemplate. (2) Find filterTemplate entry where field matches UDF field name and isUDF===true. (3) Use that entry's udfmID.",
      ...
      "NOTE: Date UDF filters CAN be combined with other UDF filter types in a single call. If you get a 400 CO.GENERAL_EXCEPTION, the cause is almost always a wrong udfmID — verify udfmID values come from filterTemplate[].udfmID, not UDF layout fieldID.",
