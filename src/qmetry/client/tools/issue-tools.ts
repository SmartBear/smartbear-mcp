...
export const ISSUE_TOOLS: QMetryToolParams[] = [
  ...
  },
  {
    ...
    hints: [
      ...
      "value can be empty string '' when no value selected (open-ended).",
      'Base structure: {"type":"string"|"list"|"numeric"|"date", "field":"<fieldKey>", "value":<value> [,"comparison":"gt"|"lt"]}',
      "",
      "TYPES:",
      "  CRITICAL — list type: value MUST be numeric IDs (except createdSystem and dfOwner which use strings — see below). NEVER use string names like 'Open', 'Critical' for ID fields — API returns 400. Get IDs from project info customListObjs.",
      '  string:  {"type":"string","field":"name","value":"Testing"}',
      '  list:    {"type":"list","field":"stateAlias","value":[<id1>,<id2>]}  — numeric IDs only; value can be ""',
      "  list (string values): createdSystem and dfOwner use STRING values (not IDs)",
      '    createdSystem: {"type":"list","field":"createdSystem","value":["QMetry","JIRA"]}',
      '    dfOwner: {"type":"list","field":"dfOwner","value":["admin","Varis Khan"]}  — use display name strings',
      '  numeric: {"type":"numeric","field":"linkedTcrCount","value":3}',
      '  date:    {"type":"date","field":"createdDate","comparison":"gt"|"lt","value":"<date-in-project-format>"}  — value can be ""',
      "",
      "FILTER FIELDS:",
      "  string:  name, entityKeyId, environmentText",
      "  list:    stateAlias, typeAlias, priorityAlias, createdByAlias, updatedByAlias, componentAlias,",
      "           affectedRelease (numeric IDs from FETCH_RELEASES_AND_CYCLES),",
      "           affectedCycles (numeric IDs; value can be ''),",
      "           createdSystem (string values: 'QMetry'|'JIRA'),",
      "           dfOwner (string display names)",
      "  numeric: linkedTcrCount, linkedRqCount, attachmentCount",
      ...
      "",
      "NOTE: Issues do NOT support archive — issues are deleted, not archived. NEVER use isArchived filter for issues.",
      "NOTE: Issues use 'affectedRelease'/'affectedCycles' — NOT 'release'/'cycle'.",
      "IDs: affectedRelease/affectedCycles from fetch_releases_and_cycles tool OR project info projects[].releases[].releaseID / builds[].buildID.",
      "     stateAlias/typeAlias/priorityAlias/createdByAlias/updatedByAlias/componentAlias IDs from project info customListObjs.*[].id (match by name).",
      ...
      "",
      "COMBINED FILTER CRITICAL RULE: When user conditions span BOTH system fields (filter) AND UDF fields (udfFilter), pass BOTH parameters in a SINGLE API call. NEVER split into two separate API calls. Example: user asks for 'status=Open AND custom field=Y' → one call with filter='[{system field}]' AND udfFilter='[{custom}]' together.",
      "=== UDF FILTER (udfFilter) — FORMAT GUIDE ===",
      "udfFilter filters issues by UDF field values. JSON array string. Multiple conditions use AND logic.",
      "CRITICAL: Every filter object MUST include 'udfmID' (projectUserFieldID). Get from 'Fetch UDF Layout' (entityType='IS'): fields[].projectUserFieldID. NEVER guess or reuse udfmID from memory/context — always fetch fresh from UDF Layout.",
