...
} from "../../../insight-hub/client/api/Project.js";
...
describe("InsightHubClient", () => {
    ...
    describe("getEndpoint method", () => {
        ...
        describe("without custom endpoint", () => {
            describe("with Hub API key (00000 prefix)", () => {
                ...
                it("should return Hub domain for custom subdomain", () => {
                    ...
                    expect(result).toBe(
                        "https://custom.insighthub.smartbear.com"
                    );
                });
        ...
        describe("with custom endpoint", () => {
            describe("Hub domain endpoints (always normalized)", () => {
                it("should normalize to HTTPS subdomain for exact hub domain match", () => {
                    const result = client.getEndpoint(
                        "api",
                        "00000key",
                        "https://api.insighthub.smartbear.com"
                    );
                    expect(result).toBe("https://api.insighthub.smartbear.com");
                });
                ...
                it("should normalize to HTTPS subdomain regardless of input protocol", () => {
                    const result = client.getEndpoint(
                        "api",
                        "00000key",
                        "http://app.insighthub.smartbear.com"
                    );
                    expect(result).toBe("https://api.insighthub.smartbear.com");
                });
                ...
                it("should normalize hub domain with port", () => {
                    const result = client.getEndpoint(
                        "api",
                        "00000key",
                        "https://custom.insighthub.smartbear.com:8080"
                    );
                    expect(result).toBe("https://api.insighthub.smartbear.com");
                });
                ...
                it("should normalize hub domain with path", () => {
                    const result = client.getEndpoint(
                        "api",
                        "00000key",
                        "https://custom.insighthub.smartbear.com/path"
                    );
                    expect(result).toBe("https://api.insighthub.smartbear.com");
                });
                ...
                it("should normalize complex subdomains to standard format", () => {
                    const result = client.getEndpoint(
                        "api",
                        "00000key",
                        "https://staging.app.insighthub.smartbear.com"
                    );
                    expect(result).toBe("https://api.insighthub.smartbear.com");
                });
            });
            ...
            describe("Custom domain endpoints (used as-is)", () => {
                ...
                it("should preserve custom domain with query parameters", () => {
                    const customEndpoint =
                        "https://custom.domain.com/api?version=1";
                    const result = client.getEndpoint(
                        "api",
                        "00000key",
                        customEndpoint
                ...
                it("should preserve custom domain with fragments", () => {
                    const customEndpoint =
                        "https://custom.domain.com/api#section";
                    const result = client.getEndpoint(
                        "api",
                        "00000key",
                        customEndpoint
            ...
            describe("edge cases", () => {
                ...
                it("should preserve custom endpoints with userinfo", () => {
                    const customEndpoint =
                        "https://user:pass@custom.domain.com";
                    const result = client.getEndpoint(
                        "api",
                        "00000key",
                        customEndpoint
                ...
                it("should normalize known domains even with userinfo", () => {
                    const result = client.getEndpoint(
                        "api",
                        "00000key",
                        "https://user:pass@app.insighthub.smartbear.com"
                    );
                    expect(result).toBe("https://api.insighthub.smartbear.com");
                });
            });
        ...
        describe("subdomain validation", () => {
            ...
            it("should handle subdomain with special characters", () => {
                ...
                expect(result).toBe(
                    "https://test-api_v2.insighthub.smartbear.com"
                );
            });
            ...
            it("should handle very long subdomains", () => {
                const longSubdomain =
                    "very-long-subdomain-name-with-many-characters";
                const result = client.getEndpoint(longSubdomain, "00000key");
                expect(result).toBe(
                    `https://${longSubdomain}.insighthub.smartbear.com`
                );
            });
        });
    });
    ...
    describe("API client initialization", () => {
        it("should initialize all required API clients", async () => {
            ...
            const { ProjectAPI } = await import(
                "../../../insight-hub/client/api/Project.js"
            );
    ...
    describe("initialization", () => {
        it("should initialize successfully with organizations and projects", async () => {
            ...
            expect(
                mockCurrentUserAPI.listUserOrganizations
            ).toHaveBeenCalledOnce();
            expect(
                mockCurrentUserAPI.getOrganizationProjects
            ).toHaveBeenCalledWith("org-1", { paginate: true });
            expect(mockCache.set).toHaveBeenCalledWith(
                "insight_hub_org",
                mockOrg
            );
            expect(mockCache.set).toHaveBeenCalledWith(
                "insight_hub_projects",
                mockProjects
            );
        });

        it("should initialize with project API key and set up event filters", async () => {
            const clientWithApiKey = new InsightHubClient(
                "test-token",
                "project-api-key"
            );
            const mockProjects = [
            ...
            expect(mockCache.set).toHaveBeenCalledWith(
                "insight_hub_current_project_event_filters",
                filteredFields
            );
        });
    ...
    describe("API methods", () => {
        ...
        describe("getRelease", () => {
            ...
            it("should handle zero accumulative_daily_users_seen", async () => {
                ...
                const result = (await client.getRelease(
                    "proj-1",
                    "rel-2"
                )) as unknown as ReleaseResponse & ReleaseStabilityData;
        ...
        describe("getEventById", () => {
            ...
            it("should return null when event not found in any project", async () => {
                ...
                mockErrorAPI.viewEventById.mockRejectedValue(
                    new Error("Not found")
                );
    ...
    describe("tool handlers", () => {
        ...
        describe("get_error tool handler", () => {
            it("should get error details with project from cache", async () => {
                ...
                mockErrorAPI.listErrorPivots.mockResolvedValue({
                    body: mockPivots,
                });
        ...
        describe("get_insight_hub_event_details tool handler", () => {
            it("should get event details from dashboard URL", async () => {
                ...
                mockCache.get.mockReturnValue(mockProjects);
                mockErrorAPI.viewEventById.mockResolvedValue({
                    body: mockEvent,
                });
        ...
        describe("get_project_event_filters tool handler", () => {
            it("should return cached event fields", async () => {
                ...
                const toolHandler = registerToolsSpy.mock.calls.find(
                    (call: any) =>
                        call[0].title === "List Project Event Filters"
                )[1];

                const result = await toolHandler({});

                expect(result.content[0].text).toBe(
                    JSON.stringify(mockEventFields)
                );
            });
        ...
        describe("update_error tool handler", () => {
            it("should update error successfully with project from cache", async () => {
                ...
                mockErrorAPI.updateErrorOnProject.mockResolvedValue({
                    status: 200,
                });
            ...
            it("should handle all permitted operations", async () => {
                ...
                for (const operation of operations) {
                    ...
                    expect(
                        mockErrorAPI.updateErrorOnProject
                    ).toHaveBeenCalledWith("proj-1", "error-1", {
                        operation,
                        severity: undefined,
                    });
                }
                    ...
                    requestedSchema: {
                        ...
                        properties: {
                            severity: {
                                ...
                                description:
                                    "The new severity level for the error",
                            },
                        },
                    },
                    required: ["severity"],
