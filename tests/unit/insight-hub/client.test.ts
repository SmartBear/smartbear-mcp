...
import { ProjectAPI, ReleaseResponse, ReleaseStabilityData } from "../../../insight-hub/client/api/Project.js";
...
const mockProjectAPI = {
  ...
  getProjectStabilityTargets: vi.fn().mockResolvedValue({
    target_stability: { value: 0.995, updated_at: "2023-01-01", updated_by_id: "user-1" },
    critical_stability: { value: 0.85, updated_at: "2023-01-01", updated_by_id: "user-1" },
...
describe("InsightHubClient", () => {
  ...
  describe("initialization", () => {
    ...
    it("should initialize with project API key and set up event filters", async () => {
      ...
      mockCache.get.mockReturnValueOnce(mockProjects).mockReturnValueOnce(null).mockReturnValueOnce(mockProjects);
    ...
    it("should throw error when no event fields found for project", async () => {
      const clientWithApiKey = new InsightHubClient("test-token", "project-api-key");
      const mockOrg = { id: "org-1", name: "Test Org" };
      const mockProjects = [{ id: "proj-1", name: "Project 1", api_key: "project-api-key" }];
  ...
  describe("API methods", () => {
    ...
    describe("getReleases", () => {
      it("should return releases from API when not cached", async () => {
        const mockReleases = [
          {
            ...
            source_control: { service: "github", commit_url: "https://github.com/org/repo/commit/abc123" },
        ...
        mockProjectAPI.listReleases.mockResolvedValue({ body: mockReleases });

        const result = await client.listReleases("proj-1", { release_stage: "production" });

        expect(mockCache.get).toHaveBeenCalledWith("insight_hub_releases_proj-1");
        expect(mockProjectAPI.listReleases).toHaveBeenCalledWith("proj-1", { release_stage: "production" });
        expect(mockCache.set).toHaveBeenCalledWith("insight_hub_releases_proj-1", enhancedReleases, 300);
      ...
      it("should return empty array when no releases found", async () => {
        ...
        expect(mockCache.set).toHaveBeenCalledWith("insight_hub_releases_proj-1", [], 300);
      ...
      it("should construct correct URL with release stage", async () => {
        // Mock cache to return null to simulate no cached data
        mockCache.get.mockReturnValueOnce(null);
        mockProjectAPI.listReleases.mockImplementation(() => ({ body: [] }));

        await client.listReleases("proj-1", { release_stage: "staging" });

        // This is testing the implementation detail that the ProjectAPI correctly constructs the URL
        expect(mockProjectAPI.listReleases).toHaveBeenCalledWith("proj-1", { release_stage: "staging" });
      });
    });
    ...
    describe("getRelease", () => {
      it("should return release from API when not cached", async () => {
        ...
        // Mock cache to return null first to simulate no cached data
        mockCache.get.mockReturnValueOnce(null);
        mockProjectAPI.getRelease.mockResolvedValue({ body: mockRelease });

        const result = await client.getRelease("proj-1", "rel-1");

        expect(mockCache.get).toHaveBeenCalledWith("insight_hub_release_rel-1");
        expect(mockProjectAPI.getRelease).toHaveBeenCalledWith("proj-1", "rel-1");
        expect(mockCache.set).toHaveBeenCalledWith("insight_hub_release_rel-1", enhancedRelease, 300);
      ...
      it("should handle zero accumulative_daily_users_seen", async () => {
        ...
        const result = await client.getRelease("proj-1", "rel-2") as unknown as ReleaseResponse & ReleaseStabilityData;
      ...
      it("should handle zero total_sessions_count", async () => {
        ...
        const result = await client.getRelease("proj-1", "rel-3") as unknown as ReleaseResponse & ReleaseStabilityData;
      ...
      it("should calculate metrics correctly when stability_target_type is session", async () => {
        ...
        mockProjectAPI.getProjectStabilityTargets.mockResolvedValueOnce({
          target_stability: { value: 0.95, updated_at: "2023-01-01", updated_by_id: "user-1" },
          critical_stability: { value: 0.9, updated_at: "2023-01-01", updated_by_id: "user-1" },
        ...
        const result = await client.getRelease("proj-1", "rel-4") as unknown as ReleaseResponse & ReleaseStabilityData;
      ...
      it("should correctly identify a release that meets target stability", async () => {
        ...
        // Override the default mockProjectAPI.getProjectStabilityTargets for this test only
        mockProjectAPI.getProjectStabilityTargets.mockResolvedValueOnce({
          target_stability: { value: 0.99, updated_at: "2023-01-01", updated_by_id: "user-1" },
          critical_stability: { value: 0.95, updated_at: "2023-01-01", updated_by_id: "user-1" },
          stability_target_type: "user" as const,
        });
        ...
        const result = await client.getRelease("proj-1", "rel-5") as unknown as ReleaseResponse & ReleaseStabilityData;
      ...
      it("should correctly identify a release that fails both critical and target stability", async () => {
        ...
        const result = await client.getRelease("proj-1", "rel-6") as unknown as ReleaseResponse & ReleaseStabilityData;
      ...
      it("should return null when release not found", async () => {
        ...
        await expect(client.getRelease("proj-1", "non-existent-release-id")).rejects.toThrow(
          "No release for non-existent-release-id found."
        );

        expect(mockProjectAPI.getRelease).toHaveBeenCalledWith("proj-1", "non-existent-release-id");
      });
    });
  ...
  describe("tool handlers", () => {
    ...
    describe("list_projects tool handler", () => {
      it("should return projects with pagination", async () => {
        ...
        client.registerTools(registerToolsSpy, getInputFunctionSpy);
        const toolHandler = registerToolsSpy.mock.calls.find((call: any) => call[0].title === "List Projects")[1];
    ...
    describe("get_error tool handler", () => {
      it("should get error details with project from cache", async () => {
        ...
        client.registerTools(registerToolsSpy, getInputFunctionSpy);
        const toolHandler = registerToolsSpy.mock.calls.find((call: any) => call[0].title === "Get Error")[1];
    ...
    describe("get_insight_hub_event_details tool handler", () => {
      it("should get event details from dashboard URL", async () => {
        ...
        client.registerTools(registerToolsSpy, getInputFunctionSpy);
        const toolHandler = registerToolsSpy.mock.calls.find((call: any) => call[0].title === "Get Event Details")[1];
    ...
    describe("list_project_errors tool handler", () => {
      it("should list project errors with filters", async () => {
        ...
        client.registerTools(registerToolsSpy, getInputFunctionSpy);
        const toolHandler = registerToolsSpy.mock.calls.find((call: any) => call[0].title === "List Project Errors")[1];
      ...
      it("should validate filter keys against cached event fields", async () => {
        ...
        mockCache.get.mockReturnValueOnce(mockProject).mockReturnValueOnce(mockEventFields);
    ...
    describe("list_releases tool handler", () => {
      it("should list releases with project from cache", async () => {
        ...
        mockCache.get.mockReturnValueOnce(mockProject).mockReturnValueOnce(null);
        mockProjectAPI.listReleases.mockResolvedValue({ body: mockReleases });

        client.registerTools(registerToolsSpy, getInputFunctionSpy);
        const toolHandler = registerToolsSpy.mock.calls.find((call: any) => call[0].title === "List Releases")[1];

        const result = await toolHandler({ releaseStage: "production" });

        expect(mockProjectAPI.listReleases).toHaveBeenCalledWith("proj-1", { release_stage: "production" });
        expect(mockCache.set).toHaveBeenCalledWith("insight_hub_releases_proj-1", enhancedReleases, 300);
        expect(result.content[0].text).toBe(JSON.stringify(enhancedReleases));
      });
      ...
      it("should list releases with explicit project ID", async () => {
        ...
        mockCache.get.mockReturnValueOnce(mockProjects).mockReturnValueOnce(null);
        mockProjectAPI.listReleases.mockResolvedValue({ body: mockReleases });

        client.registerTools(registerToolsSpy, getInputFunctionSpy);
        const toolHandler = registerToolsSpy.mock.calls.find((call: any) => call[0].title === "List Releases")[1];

        const result = await toolHandler({ projectId: "proj-1", releaseStage: "staging" });
    ...
    describe("get_release tool handler", () => {
      it("should get release details with project from cache", async () => {
        ...
        mockProjectAPI.getRelease.mockResolvedValue({ body: mockRelease });
        client.registerTools(registerToolsSpy, getInputFunctionSpy);
        const toolHandler = registerToolsSpy.mock.calls.find((call: any) => call[0].title === "Get Release")[1];
      ...
      it("should get release with explicit project ID", async () => {
        ...
        const result = await toolHandler({ projectId: "proj-1", releaseId: "rel-1" });
      ...
      it("should throw error when release not found", async () => {
        ...
        await expect(toolHandler({ releaseId: "non-existent-release-id" })).rejects.toThrow(
          "No release for non-existent-release-id found."
        );
      });
      ...
      it("should throw error when releaseId argument is missing", async () => {
        ...
        await expect(toolHandler({})).rejects.toThrow("releaseId argument is required");
      });
      ...
      it("should throw error when no project ID available", async () => {
        ...
        await expect(toolHandler({ releaseId: "rel-1" })).rejects.toThrow("No release for rel-1 found.");
      });
    });
    ...
    describe("update_error tool handler", () => {
      it("should update error successfully with project from cache", async () => {
        ...
        client.registerTools(registerToolsSpy, getInputFunctionSpy);
        const toolHandler = registerToolsSpy.mock.calls.find((call: any) => call[0].title === "Update Error")[1];
      ...
      it("should handle all permitted operations", async () => {
        ...
        for (const operation of operations) {
          ...
          expect(mockErrorAPI.updateErrorOnProject).toHaveBeenCalledWith("proj-1", "error-1", {
            operation,
            severity: undefined,
          });
        }
